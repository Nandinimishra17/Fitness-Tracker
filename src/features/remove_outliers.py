...
outlier_columns = list(df.columns[:6]) #selecting the first 6 columns only of the original dataframe because the last 3 are categorical values and obv there cant be any outliers in that
...
df[["gyr_y", "label"]].boxplot(by="label", figsize=(20,10)) #this is boxplot visualization for any one of the feature.

#here we create boxplot visualizations in one graph for first the accelerometer and then the gyroscope.
df[outlier_columns[:3] + ["label"]].boxplot(by = "label", figsize=(20, 10), layout=(1,3))
df[outlier_columns[3:] + ["label"]].boxplot(by = "label", figsize=(20, 10), layout=(1,3))

#now how to tell if the outlier marked as blacks are actually the real outliers or just normal values
...
#Chauvenet's criterion is only applicable to datasets that are normally distributed. If your dataset is not normally distributed, this method may not be suitable for identifying outliers.
